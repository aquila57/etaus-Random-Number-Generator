/* qmatch.c - FIFO Queue Match for etaus  Version 0.1.0              */
...
void enque(qfmt *tail, unsigned int outpt)
   ...
   newnode->prev = tail;
   /* The outpt parameter is the output of an etaus random number */
   /* generator cycle */
   newnode->outpt = outpt;
   } /* enque */
...
void deqenq(qfmt *head, qfmt *tail, unsigned int outpt)
   ...
   /* etaus random number generator cycle */
   node->outpt = outpt;
   } /* deqenq */
...
int main()
   ...
   unsigned int outpt;         /* 32-bit output from an etaus cycle */
   ...
   while (1)         /* infinite loop */
	 ...
	 char str[128];
         move(5,24);
         sprintf(str,"etaus with Bays-Durham Shuffle");
         addstr(str);
	 move(10,27);

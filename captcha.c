...
int main()
   ...
   *p = '\0';        /* terminate the text string */
   ...
   picrowsz = 3 * ((txtlen*15) + 80 + (etausint(et,40)));
   piccolsz = 24 + 40 + (etausint(et,40));
   ...
   while (txtindx < txtlen)
	 ...
	 while (col < 12)
	    ...
	    if (*fntp == '*')
	       ...
	       *picp++ = etauspwr(et,6);     /* 0-63 color depth */
	       *picp++ = etauspwr(et,6);     /* 0-63 color depth */
	       *picp++ = etauspwr(et,6);     /* 0-63 color depth */
	       } /* if asterisk in font */
	    col++;
